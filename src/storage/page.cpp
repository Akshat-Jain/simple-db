...
namespace simpledb::storage {
    ...
    Page::Slot Page::GetSlot(uint16_t slot_number) const {
        Page::Slot slot;
        size_t offset = Page::HEADER_SIZE + slot_number * sizeof(Page::Slot);
        memcpy(&slot, &data_[offset], sizeof(Page::Slot));
        return slot;
    }
    ...
    uint16_t Page::GetFreeSpace() const {
        ...
        const uint16_t slot_array_end = Page::HEADER_SIZE + (GetNumRecords() * sizeof(Page::Slot));
    ...
    bool Page::AddRecord(const std::vector<char>& record_data) {
        ...
        if (record_size + sizeof(Page::Slot) > GetFreeSpace()) {
        ...
        const uint16_t new_slot_offset = Page::HEADER_SIZE + num_records * sizeof(Page::Slot);
        ...
        memcpy(&data_[new_slot_offset], &new_slot, sizeof(Page::Slot));
    ...
    std::vector<char> Page::GetRecord(const Page::Slot& slot) const {
